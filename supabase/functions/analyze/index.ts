...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ourEvaluationCriteria = `
      ...
      [상한선 규칙] 5개 역량 중 90점 이상은 최대 1개만 허용한다. 2개 이상에 90점 이상을 부여하면 규칙 위반이다.
      [하한선 규칙] 5개 역량 중 최소 2개는 반드시 75점 이하여야 한다. 1개 이하이면 규칙 위반이다.
      [편차 규칙] 최고점과 최저점의 차이가 반드시 20점 이상이어야 한다. 20점 미만이면 규칙 위반이다.
      [현실적 편차 필수] 학생부 텍스트의 구체성·데이터 기반 탐구 유무·한계점을 종합하여, 강점(85~95점)과 약점(60~75점)이 뚜렷하게 대비되도록 평가한다.
      (채점 예시) 진로 연계 탐구가 구체적이면 진로역량 91점, 그러나 협업 기록이 부족하면 공동체역량 68점, 실험 데이터 검증이 없으면 탐구 깊이 72점.
