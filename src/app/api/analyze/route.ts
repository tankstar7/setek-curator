...
const prompt = `
      ...
      첨부된 학생부 PDF를 분석할 때, 아래 제시된 [우리의 평가 가이드라인]을 '절대적인 법률'로 삼아 평가를 진행하십시오. 이 기준을 벗어난 일반적인 요약은 철저히 배제합니다.
      
      ${ourEvaluationCriteria}
      
      지원 전공: ${major}
      
      [중요 분류 지시사항: 교과학습발달상황 세특 분류 기준]
      PDF의 성적표 표에 있는 '교과' 열을 기준으로 아래 3개 그룹으로 완벽하게 분류하여 JSON에 담으세요.
      - group1 (국어/수학/영어): 교과명이 국어, 수학, 영어인 모든 과목 (예: 문학, 언어와 매체, 수학I, 수학II, 영어I 등)
      - group2 (한국사/사회/과학): 교과명이 한국사, 사회(역사/도덕 포함), 과학인 모든 과목 (예: 세계사, 정치와 법, 윤리와 사상, 사회문제 탐구, 통합사회, 통합과학 등)
      - group3 (기타): 기술·가정, 제2외국어, 한문, 교양 등 나머지 모든 교과 (예: 정보, 환경, 중국어 회화 I, 체육 등)

      반드시 다음 JSON 형식으로만 출력하세요. 마크다운(\`\`\`json 등)은 절대 포함하지 마세요.
      
      {
        "basic_info": { "attendance": "출결을 성실성 기준으로 평가", "volunteer": "봉사를 나눔과 배려, 공동체역량 기준으로 평가" },
        "creative_activity": {
        ...
        "grade_trends": {
          "grade1": [ { "subject": "과목명", "sem1": "1학기 등급/성취도", "sem2": "2학기 등급/성취도" } ],
          "grade2": [ { "subject": "과목명", "sem1": "1학기 등급/성취도", "sem2": "2학기 등급/성취도" } ],
          "grade3": [ { "subject": "과목명", "sem1": "1학기 등급/성취도", "sem2": "2학기 등급/성취도" } ]
        },
        "subject_activity": {
          "grade1": {
            "group1": [ { "subject": "과목명", "summary": "팩트 위주 기록 요약", "eval": "가이드라인 기반 역량 평가", "limit": "가이드라인 기반 한계점 및 액션플랜" } ],
            "group2": [ { "subject": "과목명", "summary": "팩트 위주 기록 요약", "eval": "가이드라인 기반 역량 평가", "limit": "가이드라인 기반 한계점 및 액션플랜" } ],
            "group3": [ { "subject": "과목명", "summary": "팩트 위주 기록 요약", "eval": "가이드라인 기반 역량 평가", "limit": "가이드라인 기반 한계점 및 액션플랜" } ]
          },
          "grade2": { "group1": [], "group2": [], "group3": [] },
          "grade3": { "group1": [], "group2": [], "group3": [] }
        },
        "behavior_summary": { 
          "grade1": "...", "grade2": "...", "analysis": "...", "final_comment": "..." 
        }
      }
    `;
