...
[STEP 2: 4대 규칙 자기검증 — 반드시 순서대로 체크하고 위반 시 즉시 수정]
규칙①(상한선): 90점 이상 역량이 2개 이상이면 → 2번째부터 89점으로 강제 하향 조정
규칙②(하한선): 75점 이하 역량이 1개 이하이면 → 최저 역량 2개를 75점 이하로 강제 하향 조정
규칙③(편차): (최고점 - 최저점) < 20이면 → 최저점을 (최고점 - 20) 이하로 강제 하향 조정
규칙④(범위): 60 미만 또는 95 초과 점수가 존재하면 → 해당 점수를 범위 내(60~95)로 강제 조정

[STEP 3: 최종 검증 후 출력]
수정된 점수가 4대 규칙을 모두 만족하는지 재확인한 후 scores 필드에 기입한다.
...
interface ResponseData {
  ...
  grade_analysis: GradeAnalysis; // ← 학업성취도 정밀 분석 (신규 필드)
  subject_activity: {
  ...
  scores: {
    "학업역량": number;    // STEP 2 자기검증 통과 후 확정된 정수 (60~95)
    "진로역량": number;    // STEP 2 자기검증 통과 후 확정된 정수 (60~95)
    "공동체역량": number;  // STEP 2 자기검증 통과 후 확정된 정수 (60~95)
    "성장 주도성": number; // STEP 2 자기검증 통과 후 확정된 정수 (60~95)
    "탐구 깊이": number;   // STEP 2 자기검증 통과 후 확정된 정수 (60~95)
  };
}
`;
